...
import { ChromiumIcon, ClientIcon, DevBannerIcon, DiscordIcon, ElectronIcon, names, PatchCordIcon, settings } from ".";
...
export function makeDevBanner(state?: string): string | JSX.Element {
    ...
    const format = settings.store.format ?? "{devbannerIcon} {buildChannel} {buildNumber} ({buildHash}) | {patchcordIcon} PatchCord {patchcordVersion} ({patchcordHash})";
    ...
    const replaced = baseFormat
        ...
        .replace(/{patchcordVersion}/g, VERSION)
        .replace(/{patchcordHash}/g, gitHashShort)
        .replace(/{patchcordPlatform}/g, getVersionInfo(false))
    ...
    const parts = replaced.split(/({.*?}|__NEWLINE__)/).filter(Boolean).map((part, i) => {
        switch (part) {
            ...
            case "{patchcordIcon}":
                return <span key={`icon-patchcord-${i}`} className="vc-discord-dev-banner-icons"><PatchCordIcon /></span>;
            case "{electronIcon}":
