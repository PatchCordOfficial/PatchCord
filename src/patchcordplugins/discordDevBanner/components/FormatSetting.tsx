...
export function FormatSetting(setValue) {
    ...
    const [state, setState] = useState(format ?? "{buildChannel} {buildNumber} ({buildHash}) | PatchCord {patchcordVersion} ({patchcordHash})");
    ...
    return (
        <section>
            ...
            <textarea
                ...
                placeholder="{buildChannel} {buildNumber} ({buildHash}) | PatchCord {patchcordVersion} ({patchcordHash})"
                rows={1}
                ref={el => {
                    if (el) {
