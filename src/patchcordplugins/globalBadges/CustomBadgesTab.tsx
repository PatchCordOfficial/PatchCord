...
function BadgeCard({ userId }: { userId: string; }) {
    ...
    const badges = getGlobalBadges()[userId] ?? [];
...
function CustomBadgesTab() {
    ...
    console.log("CustomBadges: render", { search, sortMode, refreshing });

    // loadBadges() calls UserProfileStore.emitChange() once it has fresh
    // data, so subscribing here is enough to make this tab re-render live
    // whenever badges refresh (on the interval, on profile open, or when
    // the Refresh button below is clicked) without needing a remount.
    const [, forceUpdate] = useState(0);
    useEffect(() => {
        const onChange = () => forceUpdate(n => n + 1);
        UserProfileStore.addChangeListener(onChange);
        return () => UserProfileStore.removeChangeListener(onChange);
    }, []);

    const badgesMap = getGlobalBadges();
    const userIds = useMemo(
        () => Object.keys(badgesMap).filter(id => (badgesMap[id]?.length ?? 0) > 0),
        [badgesMap]
    );

    console.log("CustomBadges: badge map state", { badgeUsers: userIds.length, badgesMapKeys: Object.keys(badgesMap).length, badgesMap });
    ...
    const refresh = async () => {
        console.log("CustomBadges: refresh start");
        setRefreshing(true);
        ...
        console.log("CustomBadges: refresh complete", {
            badgesLoaded: Object.keys(getGlobalBadges()).length,
            success,
        ...
        <SettingsTab>
            ...
            {!userIds.length && (
                <div className={classes(Margins.top16, cl("empty"))}>
                    <Paragraph>
                        {getLastLoadError()
                            ? `Failed to load badges: ${getLastLoadError()}`
            ...
            <div className={classes(Margins.top16, cl("grid"))}>
                {filteredUserIds.map(id => <BadgeCard key={id} userId={id} />)}
            </div>
        </SettingsTab>
    );
}
