...
export async function loadBadges() {
    ...
    console.log("CustomBadges: loadBadges start", { apiUrl: settings.store.apiUrl, resolvedUrl: url });

    const Native = VencordNative.pluginHelpers.GlobalBadges as PluginNative<typeof import("./native")> | null;
    let globalBadges: { users?: Record<string, unknown>; };
    try {
        let data: Record<string, any>;

        if (Native?.fetchBadgesJson) {
            data = await Native.fetchBadgesJson(url);
        } else {
            // Fallback for environments where the native bridge is unavailable.
        ...
        globalBadges = data.users ? data as { users: Record<string, unknown>; } : { users: data as Record<string, unknown> };
        lastLoadError = null;
    } catch (e) {
        ...
        new Logger("GlobalBadges").error(
            `Failed to load badges from ${url}. If you changed the "API URL" setting, make sure it points to a GlobalBadges-compatible server (default: https://patchcord.itssolar.dev/badges.json) or a static badges.json file.`,
            e
        );
        return false;
    }
    ...
    for (const key of Object.keys(rawUsers)) {
        ...
        filteredUsers[key] = rawBadges
            ...
            .map((badge, idx) => {
                ...
                const allowed = !mod || mod === "patchcord";
                if (!allowed) {
                    console.log("CustomBadges: skipping non-PatchCord badge", { userId: key, badge, mod });
                    return null;
                }
