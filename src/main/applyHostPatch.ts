    ...
    try {
        ...
        for (let i = undo.length - 1; i >= 0; i--) {
            try {
                ...
                console.error("[PatchCord] Rollback step failed", cleanupErr);
            }
        }
        throw err;
    }
    ...
    try {
        for (const name of readdirSync(discordPath)) {
            ...
            try {
                ...
                console.error("[PatchCord] Skipping unreadable sibling", name, statErr);
                continue;
            }
            if (!isDir) continue;
        ...
        console.error("[PatchCord] Failed to scan for sibling versions", err);
        return null;
    }
